...
void customerInfo::edited(QTableWidgetItem * w){
   ...
   // qDebug() << "editing happened at " << w->row() << " and " << w->column();
    qu.prepare("UPDATE  `Contacts` SET `name`=:name, `company`=:company, `address1`=:address1, `address2`=:address2, `city`=:city, `state`=:state, `zip`=:zip, `email`=:email, `fax`=:fax, `phone1`=:phone1, `phone2`=:phone2 WHERE name = '"+name+"'");
...
void customerInfo::addContact(QString name, QString company, QString a1, QString a2, QString city, QString state, QString zip, QString email, QString fax, QString phone1, QString phone2){
    QSqlQuery qu;
    qu.prepare("INSERT INTO `Contacts` VALUES (:name, :company, :address1, :address2, :city, :state, :zip, :email, :fax, :phone1, :phone2) ");
